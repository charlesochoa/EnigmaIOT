...
//#endif
    DEBUG_DBG ("Clock set.");
    DEBUG_DBG ("Connect to MQTT server: server %s, port %d, user %s, pass %s, topic %s",
           mqttgw_config.mqtt_server, mqttgw_config.mqtt_port, mqttgw_config.mqtt_user, mqttgw_config.mqtt_pass, gwTopic.c_str ());
...
bool GwOutput_MQTT::addMQTTqueue (const char* topic, char* payload, size_t len, bool retain) {
  ...
  // DEBUG_DBG ("%d MQTT messages queued Len:%d %s %d %s", mqtt_queue.size (),
   //      len,
   //      message.topic,
